...
from .model.ApiRtn import LoginRtn, MsgRtn
...
def login_buf2Resp(buf, login_aes_key):
    ...
    # 登录异常处理
    if -301 == loginRes.result.code:        
        #tempStr='登陆结果:\ncode:{}\n即将尝试切换DNS重新登陆!'.format(loginRes.result.code)                                                     # DNS解析失败,请尝试更换idc
        #logger.error(tempStr)                    
        returnStr= LoginRtn(loginRes.result.code,'即将尝试切换DNS重新登陆','','','','','')
    elif -106 == loginRes.result.code:  
        #tempStr='登陆结果:\ncode:{}\nError msg:{}\n'.format(loginRes.result.code, loginRes.result.err_msg)                                                        
        # 需要在IE浏览器中滑动操作解除环境异常/扫码、短信、好友授权(滑动解除异常后需要重新登录一次)
        # 打开IE,完成授权
        #tempStr+='\n请在浏览器授权后重新登陆!'
        #logger.error(tempStr)
        returnStr= LoginRtn(loginRes.result.code,loginRes.result.err_msg,'','','','','')
    ...
    elif -3 == loginRes.result.code:      
        #tempStr='登陆结果:\ncode:{}\nError msg:{}\n'.format(loginRes.result.code, loginRes.result.err_msg.msg[loginRes.result.err_msg.msg.find(
            #'<Content><![CDATA[')+len('<Content><![CDATA['):loginRes.result.err_msg.msg.find(']]></Content>')])                                                         # 其他登录错误
        #logger.error(tempStr)
        returnStr= LoginRtn(loginRes.result.code,loginRes.result.err_msg,'','','','','')
    elif -34 == loginRes.result.code:      
        #tempStr='登陆结果:\ncode:{}\nError msg:{}\n'.format(loginRes.result.code, loginRes.result.err_msg.msg[loginRes.result.err_msg.msg.find(
            #'<Content><![CDATA[')+len('<Content><![CDATA['):loginRes.result.err_msg.msg.find(']]></Content>')])                                                         # 其他登录错误
        #logger.error(tempStr)
        returnStr= LoginRtn(loginRes.result.code,loginRes.result.err_msg,'','','','','')
        ...
        # 初始化db
        Util.init_db()
        returnStr= LoginRtn(loginRes.result.code,'登录成功',Util.sessionKey,Util.uin,Util.wxid,loginRes.accountInfo.nickName,loginRes.accountInfo.Alias)
    return returnStr
    ...
    for i in range(res.msg.cntList):
        if 5 == res.msg.tag2[i].type:                                                                       # 未读消息
            ...
            if 10002 == msg.type and 'weixin' ==  msg.from_id.id:                                           # 过滤系统垃圾消息                                                          
                ...
                # 接入插件
                plugin.dispatch(msg)   
                msg = MsgRtn(res.msg.tag2[i].type, msg.type,
                             msg.serverid, msg.createTime, msg.from_id.id, msg.to_id.id, msg.raw.content)
                msg_list.append(msg)
...
def revoke_msg_buf2resp(buf):
    ...
    return res.res.code, res.response_sys_wording
...
def set_chatroom_announcement_buf2resp(buf):
    ...
    return res.res.code, res.res.message

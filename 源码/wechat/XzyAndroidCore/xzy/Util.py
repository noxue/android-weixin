...
def get_contact(contact_type):
    ...
    if contact_type & CONTACT_TYPE_FRIEND:                                                  # 返回好友列表
        cur.execute("select * from contact where wxid not like '%%@chatroom' and wxid not like 'gh_%%' and (type & 8) = 0 and (type & 1)")
        rows = rows + cur.fetchall()
    if contact_type & CONTACT_TYPE_CHATROOM:                                                # 返回群聊列表
        cur.execute("select * from contact where wxid like '%%@chatroom' and (type & 1)")
        rows = rows + cur.fetchall()
    if contact_type & CONTACT_TYPE_OFFICAL:                                                 # 返回公众号列表
        cur.execute("select * from contact where wxid like 'gh_%%' and (type & 1)")
        rows = rows + cur.fetchall()
    if contact_type & CONTACT_TYPE_BLACKLIST:                                               # 返回黑名单列表
        cur.execute("select * from contact where wxid not like '%%@chatroom' and wxid not like 'gh_%%' and (type & 8)")
        rows = rows + cur.fetchall()
    if contact_type & CONTACT_TYPE_DELETED:                                                 # 返回已删除好友列表(非主动删除对方?)
        cur.execute("select * from contact where wxid not like '%%@chatroom' and wxid not like 'gh_%%' and (type & 1) = 0")
        rows = rows + cur.fetchall()
    return rows
